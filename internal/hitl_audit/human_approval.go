...
type HITLRequest struct {
	...
	// through purely for the audit log (hitl_audit.go) - the prompt UI
...
func (h *HITLManager) decide(req HITLRequest, mode HITLMode) HITLResponse {
	...
	case HITLModeAutoReject:
		log.PrintSystem(fmt.Sprintf("[hitl] auto-reject: %s (%s) - %s",
			req.ToolName, req.RiskLevel, req.Reason))
		return HITLResponse{Decision: HITLReject}

	case HITLModeNotifyOnly:
		log.PrintSystem(fmt.Sprintf("[hitl] NOTIFY: %s (%s) - %s",
			req.ToolName, req.RiskLevel, req.Reason))
		return HITLResponse{Decision: HITLApprove}
	...
	case HITLModeInteractive:
		...
		if !isStdinTTY() {
			log.PrintSystem(fmt.Sprintf("[hitl] no tty available, auto-rejecting %s (%s) - %s; "+
...
func (h *HITLManager) promptInteractive(req HITLRequest) HITLResponse {
	...
	for {
		...
		if err != nil {
			...
			log.PrintSystem("[hitl] stdin unavailable, defaulting to reject")
			return HITLResponse{Decision: HITLReject}
		}
...
// This avoids a circular import while still allowing the hitl_audit package
