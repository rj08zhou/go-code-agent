...
// internal/logging; this file installs the file-side handler so each
...
func renderSessionLogReader(r io.Reader) string {
	...
	first := true
	for _, k := range logging.KnownKinds {
		if kinds[k] == 0 {
			continue
		}
		if !first {
	...
	for k, c := range kinds {
		if slices.Contains(logging.KnownKinds, k) {
			continue
		}
		if !first {
			b.WriteString(", ")
