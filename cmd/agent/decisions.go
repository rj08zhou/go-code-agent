...
//
// log.PrintDecision surfaces every autonomous action (planning, context
...
// initDecisionLog wires log.PrintDecision to also persist each event to the
...
func initDecisionLog() {
	log.SetDecisionSink(appendDecision)
}
...
func renderDecisions() string {
	...
	first := true
	for _, k := range []string{"plan", "context", "memory", "judge", "reflect"} {
		if counts[k] == 0 {
			continue
		}
		if !first {
